...
<body>
	...
	<div style="margin-top: 10px;">
		<form class="formCondition" id="search_condition"
			action="${path}/sys/project/authUser/dataForPage.action">
			<!--查询条件-->
			<div>
				<input type="hidden" id="pgNo" name="pgNo" value="1" />
				<div class="form-group" class="col-sm-4 ">
					<label for="firstname" class="col-sm-1 control-label">用户密码</label>
					<div class="col-sm-2">
						<input type="text" class="form-control"
							name="authUser.userPassword" value="${authUserModel.authUser.userPassword}"
							placeholder="请输入">
					</div>
				</div>
				<div class="form-group" class="col-sm-4 ">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control"
							name="authUser.userName" value="${authUserModel.authUser.userName}"
							placeholder="请输入">
					</div>
				</div>
				<div class="form-group" class="col-sm-4 ">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control"
							name="authUser.userAccount" value="${authUserModel.authUser.userAccount}"
							placeholder="请输入">
					</div>
				</div>
			</div>
			...
			<div style="margin: 10px 0px; clear: both;" class="col-sm-12">
				...
				<button type="button" class="btn btn-default" data-toggle="modal"
					...
					onclick="
							...
							$('#formInit').panel({    
								    href:'${path}/sys/project/authUser/form.action', 
								});
			         	">添加</button>
			...
			<div style="margin: 10px 0px; clear: both;" class="col-sm-12">
				<button type="button" class="btn btn-primary"
					onclick="
			         	location.href='${path}/sys/project/authUser/ExportModel.action'
			         ">下载Excel模板</button>

				<button type="button" class="btn btn-primary"
					onclick="
			         	location.href='${path}/sys/project/authUser/ExportDatas.action'
			         ">导出数据到Excel</button>
			...
			<div class="modal-content">
				...
				<div id=modal_content class="modal-body" style="overflow: hidden;">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary"
							onclick="
	        		ajaxSubmit('#dataForm','${path}/sys/project/authUser/save.action');
	        		">保存</button>
					</div>
				</div>
			</div>
	...
	<div class="rows">
		<div class="panel panel-default panel-primary col-md-12">
			...
			<div class="panel-body">
				...
				<!-- 数据内容开始了 ----------------------------------------->
				<form id="datasForm"
					action="${path}/sys/project/authUser/delete.action" method="post">
					<table class="table table-striped">
						...
						<tbody>
							<c:forEach items="${page.item}" var="authUser">
								<tr>
									<td><input type="checkbox" name="ids" value="${authUser.id}" />
										<a
										href="${path}/sys/project/authUser/delete.action?ids=${authUser.id}">删除</a>
										<a href="#"
										onclick="
		        			         		$('#formDialog').modal({show:true});
		        							$('#formInit').panel({    
		        								    href:'${path}/sys/project/authUser/form.action?authUser.id=${authUser.id}', 
		        								});
	                        			">编辑</a>
									</td>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div id=modal_content class="modal-body">
					<!-- 数据导入主体 -->
					<iframe src="${path_page}/sys/project/authUser/importDatas.jsp"></iframe>
					<!-- 数据导入主体  end-->
				</div>
			</div>
		</div>
