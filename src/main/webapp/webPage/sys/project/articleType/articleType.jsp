...
<body>
	...
	<div style="margin-top: 10px;">
		<form class="formCondition" id="search_condition"
			action="${path}/sys/project/articleType/dataForPage.action">
			<!--查询条件-->
			<!-- 查询条件-------------------------------------------------------------开始------------------ -->
			<div class="rows">
							...
							<div class="form-group col-sm-4 ">
								...
								<div class="col-sm-6">
									<input type="text" class="form-control"
										name="articleType.name" value="${articleTypeModel.articleType.name}"
										placeholder="请输入">
								</div>
							</div>
							<div class="form-group col-sm-4 ">
								...
								<div class="col-sm-6">
									<input type="text" class="form-control"
										name="articleType.state" value="${articleTypeModel.articleType.state}"
										placeholder="请输入">
								</div>
							</div>
						</div>
			...
			<div style="margin: 10px 0px; clear: both;" class="col-sm-12">
				...
				<button type="button" class="btn btn-default" data-toggle="modal"
					...
					onclick="
							...
							$('#formInit').panel({    
								    href:'${path}/sys/project/articleType/form.action', 
								});
			         	">添加</button>
			...
			<div style="margin: 10px 0px; clear: both;" class="col-sm-12">
				<button type="button" class="btn btn-primary"
					onclick="
			         	location.href='${path}/sys/project/articleType/ExportModel.action'
			         ">下载Excel模板</button>

				<button type="button" class="btn btn-primary"
					onclick="
			         	location.href='${path}/sys/project/articleType/ExportDatas.action'
			         ">导出数据到Excel</button>
			...
			<div class="modal-content">
				...
				<div id=modal_content class="modal-body" style="overflow: hidden;">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary"
							onclick="
	        		ajaxSubmit('#dataForm','${path}/sys/project/articleType/save.action');
	        		">保存</button>
					</div>
				</div>
			</div>
	...
	<div class="rows">
		<div class="panel panel-default panel-primary col-md-12">
			...
			<div class="panel-body">
				...
				<!-- 数据内容开始了 ----------------------------------------->
				<form id="datasForm"
					action="${path}/sys/project/articleType/delete.action" method="post">
					<table class="table table-striped">
						...
						<tbody>
							<c:forEach items="${page.item}" var="articleType">
								<tr>
									<td><input type="checkbox" name="ids" value="${articleType.id}" />
										<a
										href="${path}/sys/project/articleType/delete.action?ids=${articleType.id}">删除</a>
										<a href="#"
										onclick="
		        			         		$('#formDialog').modal({show:true});
		        							$('#formInit').panel({    
		        								    href:'${path}/sys/project/articleType/form.action?articleType.id=${articleType.id}', 
		        								});
	                        			">编辑</a>
									</td>
									<td>${articleType.name}</td>
									<td>${articleType.state}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div id=modal_content class="modal-body">
					<!-- 数据导入主体 -->
					<iframe src="${path_page}/sys/project/articleType/importDatas.jsp"></iframe>
					<!-- 数据导入主体  end-->
				</div>
			</div>
		</div>
