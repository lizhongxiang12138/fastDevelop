...
<!--引入CSS-->
<link rel="stylesheet" type="text/css"
	href="${pathResource_tools }/webuploader/webuploader.css">
...
<body>
	...
	<form:form id="dataForm" method="POST" modelAttribute="articleModel"
		class="form-horizontal form-inline" role="form">
		<input type="hidden" name="article.id"
			value="${articleModel.article.id}" />
		...
		<div class="form-group" style="margin-top: 10px; width: 100%">
			...
			<div style="width: 75%; float: left;">
				<input type="hidden" id="titleImageID" name="article.titleImageID"
					value="${articleModel.article.titleImageID}">
				<div id="uploader" class="wu-example">
					<!--用来存放文件信息-->
					<div id="thelist" class="uploader-list">

						<c:if test="${!empty articleModel.article.titleImageID}">
							<!-- S-标题图片信息 -->
							<div id="${articleModel.article.titleImageID}"
				...
				$(function() {
					...
					function deleteTitleImg() {
						if ('' != $.trim(titleImgID)) {
							$
									.ajax({
										type : 'get',
										url : '${pathSys}/project/article/deleteImgGroup.action?ids='
												+ titleImgID,
										dataTypt : 'json',
					...
					var uploader = WebUploader
							.create({
								...
								// 文件接收服务端。
								server : '${pathSys}/project/article/uploadeImg.action',
		...
		<div class="form-group" style="margin-top: 10px; width: 100%">
			<label for="firstname" class="control-label"
				style="width: 20%; float: left;">文章栏目ID：</label>
			<div class="col-sm-4">
				<my:Select name="article.typeID"
	         		className="form-control easyui-validatebox"
	         		dataOptions="required:true"
					beanClassName="com.my.project.entity.ArticleType" value="id"
					valueName="name" where="1=1"
					selectValue="${articleModel.article.typeID }" />
			</div>
		</div>
		<!-- E-文章栏目选择end -->
		...
		<div class="form-group" style="margin-top: 10px; width: 100%">
			<label for="firstname" class="control-label"
				style="width: 20%; float: left;">标题：</label>
			<div class="col-sm-4">
				<input type="text" class="form-control easyui-validatebox"
					name="article.title" data-options="required:true"
					validType="data_baseType" value="${articleModel.article.title}">
			</div>
		</div>
		...
		<div class="form-group" style="margin-top: 10px; width: 100%">
			<label for="firstname" class="control-label"
				style="width: 20%; float: left;">内容：</label>
			<div style="width: 75%; float: left;">
				<!-- 加载编辑器的容器 -->
				<script id="container" name="article.content" type="text/plain"
					style="height: 300px;">
					${articleModel.article.content}
				</script>
				<!-- 配置文件 -->
				<script type="text/javascript"
					src="${path_page }/sys/project/article/ueditor/ueditor.config.js"></script>
				<!-- 编辑器源码文件 -->
				<script type="text/javascript"
					src="${path_page }/sys/project/article/ueditor/ueditor.all.js"></script>
				<!-- 实例化编辑器 -->
				<script type="text/javascript">
							...
							.on(
									...
									function(file, data) {
										...
										var $percent = $li
												.find('.progress-bar');
										$percent
												.replaceWith('<span>文件上传成功</span>');
										/* 添加img对应id的input */
										$li
												.append('<input type="hidden" name="imgGroup" value="'+data.documentId+'">')
									});
					...
					$('#right_menu').menu(
							{
								onClick : function(item) {
									...
									if ("editDescription" == id) {
										...
										$('#imgRemarkPage').attr(
												'src',
												'${pathSys}/project/article/remarkImgPage.action?document.id='
														+ imgId);
									}
								}
							});
	...
	<p style="text-align: center;">
		...
		<button type="button"
			onclick="ajaxSubmit('#dataForm','${path}/sys/project/article/save.action');">保存</button>
	</p>
	<!-- E-提交按钮 -->
