...
<body>
	...
	<div style="margin-top: 10px;">
		<form class="formCondition" id="search_condition"
			action="${pathSys}/project/role/dataForPage.action">
			<!--查询条件-->
			<div>
				...
				<div class="form-group" class="col-sm-4 ">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control" id="firstname"
							name="role.roleName" value="${roleModel.role.roleName}"
							placeholder="请输入">
					</div>
				</div>
			</div>
			...
			<div style="margin: 10px 0px; clear: both;" class="col-sm-12">
				...
				<button type="button" class="btn btn-default" data-toggle="modal"
					...
					onclick="
							...
							$('#formInit').panel({    
								    href:'${path }/sys/project/role/form.action', 
								});
			         	">添加</button>
			...
			<div style="margin: 10px 0px; clear: both;" class="col-sm-12">
				<button type="button" class="btn btn-primary"
					onclick="
			         	location.href='${path }/sys/project/role/ExportModel.action'
			         ">下载Excel模板</button>

				<button type="button" class="btn btn-primary"
					onclick="
			         	location.href='${path }/sys/project/role/ExportDatas.action'
			         ">导出数据到Excel</button>
			...
			<div class="modal-content">
				...
				<div id=modal_content class="modal-body" style="overflow: hidden;">
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary"
							onclick="
	        		ajaxSubmit('#dataForm','${path }/sys/project/role/save.action');
	        		">保存</button>
					</div>
				</div>
			</div>
	...
	<div class="rows">
		<div class="panel panel-default panel-primary col-md-12">
			...
			<div class="panel-body">
				...
				<!-- 数据内容开始了 ----------------------------------------->
				<form id="datasForm"
					action="${path }/sys/project/role/delete.action" method="post">
					<table class="table table-striped">
						...
						<tbody>
							<c:forEach items="${page.item }" var="role">
								<tr>
									<td><input type="checkbox" name="ids" value="${role.id }" />
										<a
										href="${path }/sys/project/role/delete.action?ids=${role.id}">删除</a>
										<a href="#"
										onclick="
		        			         		$('#formDialog').modal({show:true});
		        							$('#formInit').panel({    
		        								    href:'${path }/sys/project/role/form.action?role.id=${role.id }', 
		        								});
	                        			">编辑</a>
	                        			<a href="#"
	                        				onclick="$('#permissionsDialog').modal({show:true});
	                        					$('#permissionsFrame').
	               									attr('src','${pathSys}/project/role/permissions.action?role.id=${role.id }&role.roleName=${role.roleName} &request=page')
	                        				"
	                        				>权限管理</a>
									</td>
									<td>${role.roleName }</td>
	...
	<!-- 权限管理对话框 ---------------------------------------------------开始-------------------->
	<div id="permissionsDialog" class="modal fade" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" style="overflow: hidden;">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div id=modal_content class="modal-body" style="overflow: hidden;">
					<iframe id="permissionsFrame" src="" width="100%" height="350px"></iframe>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div id=modal_content class="modal-body">
					<!-- 数据导入主体 -->
					<iframe src="${sys_project}/role/importDatas.jsp"></iframe>
					<!-- 数据导入主体  end-->
				</div>
			</div>
		</div>
